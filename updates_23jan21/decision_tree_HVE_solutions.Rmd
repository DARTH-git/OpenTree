...
Evaluate outcomes of decision tree

```{r, warning = F}
# define a model name
treeName <- "HVE" # placeholdername for model ... use the File commands on OpenTree to create NEW, Open or Save OpenTrees.
...
# Calculate total costs and QALYs for each strategy 
 for (i in 1:length(v_names_str)) {
  v_total_qaly[i] <- eval_num(
    df_tree_results$v_prob[i]) %*% eval_num(df_tree_results$v_payoff1[i])
  v_total_cost[i] <- eval_num(
    df_tree_results$v_prob[i]) %*% eval_num(df_tree_results$v_payoff2[i])
 }
