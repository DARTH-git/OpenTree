...
openTree(dirname = here("updates_23jan21", "Helloworld.json"), 
         libpath = OpenTree.libpath)
```

```{r,}
# remove.packages("OpenTree")
devtools::install_github("DARTH-git/OpenTreep-kg")
library(OpenTree)
createTree(fileName = "Helloworld", dirname = getwd())
library(here)
openTree(here("updates_23jan21", "Helloworld.json"))
```

```{r}
library(OpenTree)
create_tree(file_name = "Helloworld.json", dir_name = getwd())
open_tree(file_name = "Helloworld.json", dir_name = getwd())
```
